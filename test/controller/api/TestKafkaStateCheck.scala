...
class TestKafkaStateCheck extends CuratorAwareTest with KafkaServerInTest with MockitoSugar {
  ...
  private[this] def createCluster() = {
    val future = kafkaManagerContext.get.getKafkaManager.addCluster(
      testClusterName, "2.4.1", kafkaServerZkPath, kafkaZkRoot = None, jmxEnabled = false, pollConsumers = true, filterConsumers = true, jmxUser = None, jmxPass = None, jmxSsl = false, tuning = Option(kafkaManagerContext.get.getKafkaManager.defaultTuning), securityProtocol = "PLAINTEXT", saslMechanism = None, jaasConfig = None
    )
